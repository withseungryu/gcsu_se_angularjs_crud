...
class DBController {
   ...
   function runSelectQueryArray($query) //Used to run a select query and return array of rows
     ...
     *   new record being inserted
     */
    function createUpdateString($table_name, $post) {
        ...
        $combo=array_combine($keys, $valarray);
        ...
        return "UPDATE $table_name SET $str WHERE privilegeID='$pvID'";
    }

    function runSimpleUpdateQuery($query)
    {
        //$result be TRUE even if 0 records were deleted.
        $result = mysqli_query($this->conn,$query);
        $affected_rows= mysqli_affected_rows($this->conn);
        ...
        if ($result === TRUE) {
            ...
            $status = "error";
            $affected_rows="";
            $statusText ="Error Updating record: ' . $this->conn->error.'";
            $data = false;
        }
        $output = array();
        $output['status'] = $status;
